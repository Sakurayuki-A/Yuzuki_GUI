...
SweepGradientEffect* g_created_effect = nullptr;

HRESULT __stdcall create_sweep_effect(IUnknown** effect_impl) {
    auto* effect = new SweepGradientEffect();
    g_created_effect = effect;
    *effect_impl = static_cast<ID2D1EffectImpl*>(effect);
    return *effect_impl ? S_OK : E_OUTOFMEMORY;
}

const char kSweepShader[] = R"(
...
cbuffer cb : register(b0) {
    ...
    float unused_pad;
    float4 colors[8];
    float4 stop_pos[2];  // positions packed 4-per-float4 (x,y,z,w)
};

float stop_at(int i) {
    int row = i >> 2;
    int col = i & 3;
    return col == 0 ? stop_pos[row].x
         : col == 1 ? stop_pos[row].y
         : col == 2 ? stop_pos[row].z
                    : stop_pos[row].w;
}
...
D2D_PS_ENTRY(PS) {
    ...
    // region beyond the arc wrap back to the first color, leaving a stray blob opposite the arc).
    float full = sweep_angle >= 6.28 ? 1.0 : 0.0;
    float t = full > 0.0 ? frac(angle / sweep_angle) : min(angle / sweep_angle, 1.0);
    // Seam-free wrap: the last `blend` fraction of a full sweep fades back to the FIRST
    // stop color via smoothstep (zero slope at both ends), so no hard band at 0/360 deg.
    int n = (int)(stop_count + 0.5);
    n = clamp(n, 1, 8);
    float blend = full * 0.20;
    float scale = 1.0 - blend;
    // Piecewise-linear interpolation across the stops over [0, scale]; beyond the last
    // position the last color clamps (partial arcs keep their end color).
    float u = saturate(t / max(scale, 1e-6));
    float4 result = colors[0];
    float prev = stop_at(0);
    for (int i = 1; i < 8; ++i) {
        if (i >= n) break;
        float pi = stop_at(i);
        float seg = saturate((u - prev) / max(pi - prev, 1e-6));
        result = lerp(result, colors[i], seg);
        if (u <= pi) break;
        prev = pi;
    }
    float back = saturate((t - scale) / (blend + 1e-6));
    back = back * back * (3.0 - 2.0 * back);
    result = lerp(result, colors[0], back);
    return result;
})";

}  // namespace

HRESULT SweepGradientEffect::register_effect(ID2D1Factory1* factory) {
    static const wchar_t kXml[] =
        L"<?xml version='1.0'?>"
        L"<Effect>"
        L"<Property name='DisplayName' type='string' value='Yuzuki Sweep Gradient'/>"
        L"<Property name='Author' type='string' value='Yuzuki'/>"
        L"<Property name='Category' type='string' value='Sample'/>"
        L"<Property name='Description' type='string' value='Sweep gradient fill.'/>"
        L"<Inputs>"
        L"<Input name='Source'/>"
        L"</Inputs>"
        L"</Effect>";
    return factory->RegisterEffectFromString(kSweepGradientClsid, kXml, nullptr, 0,
                                             &create_sweep_effect);
}

HRESULT SweepGradientEffect::create(ID2D1DeviceContext* context, ID2D1Effect** out_effect,
                                    SweepGradientEffect** out_impl) {
    g_created_effect = nullptr;
    Microsoft::WRL::ComPtr<ID2D1Effect> effect;
    HRESULT hr = context->CreateEffect(kSweepGradientClsid, &effect);
    if (FAILED(hr)) return hr;
    if (!g_created_effect) return E_FAIL;
    *out_effect = effect.Detach();
    *out_impl = g_created_effect;
    return S_OK;
}

HRESULT SweepGradientEffect::Initialize(ID2D1EffectContext* context,
                                        ID2D1TransformGraph* graph) {
    context_ = context;
    // Compile at runtime from the embedded source: the shader evolves with the codebase
    // and can never go stale (the old prebuilt-CSO blob had to be regenerated by hand
    // on every edit). d3dcompiler is already a linked dependency.
    ...
    HRESULT hr = D3DCompile(kSweepShader, std::strlen(kSweepShader), "yuzuki_sweep.hlsl", defines,
                            nullptr, "PS", "ps_4_0", D3DCOMPILE_OPTIMIZATION_LEVEL3, 0, &blob,
                            &errors);
    if (FAILED(hr)) return hr;
    hr = context_->LoadPixelShader(kSweepShaderId,
...
HRESULT SweepGradientEffect::PrepareForRender(D2D1_CHANGE_TYPE change_type) {
    (void)change_type;
    if (!draw_info_) return E_FAIL;
    HRESULT hr = draw_info_->SetPixelShader(kSweepShaderId, D2D1_PIXEL_OPTIONS_NONE);
    if (FAILED(hr)) return hr;
    return draw_info_->SetPixelShaderConstantBuffer(
        reinterpret_cast<const BYTE*>(&params_), static_cast<UINT32>(sizeof(params_)));
}

HRESULT SweepGradientEffect::SetGraph(ID2D1TransformGraph* graph) {
    return graph->SetSingleTransformNode(this);
}

HRESULT SweepGradientEffect::SetDrawInfo(ID2D1DrawInfo* draw_info) {
    draw_info_ = draw_info;
    return S_OK;
}

HRESULT SweepGradientEffect::MapInputRectsToOutputRect(
    const D2D1_RECT_L* input_rects, const D2D1_RECT_L* input_opaque_sub_rects, UINT32 input_count,
    D2D1_RECT_L* output_rect, D2D1_RECT_L* output_opaque_sub_rect) {
(void)input_opaque_sub_rects;
    (void)input_count;
    *output_rect = input_rects[0];
    *output_opaque_sub_rect = *output_rect;
    return S_OK;
}

HRESULT SweepGradientEffect::MapOutputRectToInputRects(const D2D1_RECT_L* output_rect,
                                                       D2D1_RECT_L* input_rects,
                                                       UINT32 input_count) const {
    for (UINT32 i = 0; i < input_count; ++i) {
        input_rects[i] = *output_rect;
    }
    return S_OK;
}

HRESULT SweepGradientEffect::MapInvalidRect(UINT32 input_index, D2D1_RECT_L invalid_input_rect,
                                            D2D1_RECT_L* invalid_output_rect) const {
    (void)input_index;
    *invalid_output_rect = invalid_input_rect;
    return S_OK;
}

UINT32 SweepGradientEffect::GetInputCount() const {
    return 1;
}

HRESULT SweepGradientEffect::set_params(const SweepParams& params) {
    params_ = params;
    return S_OK;
}

HRESULT SweepGradientEffect::QueryInterface(const IID& iid, void** object) {
    if (iid == __uuidof(SweepGradientEffect)) {
        AddRef();
        *object = this;
        return S_OK;
    }
    if (iid == __uuidof(ID2D1EffectImpl)) {
        AddRef();
        *object = static_cast<ID2D1EffectImpl*>(this);
        return S_OK;
    }
    if (iid == __uuidof(ID2D1DrawTransform)) {
        AddRef();
        *object = static_cast<ID2D1DrawTransform*>(this);
        return S_OK;
    }
    if (iid == __uuidof(ID2D1Transform)) {
        AddRef();
        *object = static_cast<ID2D1Transform*>(this);
        return S_OK;
    }
    if (iid == __uuidof(ID2D1TransformNode)) {
        AddRef();
        *object = static_cast<ID2D1TransformNode*>(this);
        return S_OK;
    }
    if (iid == __uuidof(ISweepGradient)) {
        AddRef();
        *object = static_cast<ISweepGradient*>(this);
        return S_OK;
    }
    if (iid == IID_IUnknown) {
        AddRef();
        *object = static_cast<IUnknown*>(static_cast<ISweepGradient*>(this));
        return S_OK;
    }
    *object = nullptr;
...
ULONG SweepGradientEffect::AddRef() {
    return ++ref_count_;
}

ULONG SweepGradientEffect::Release() {
